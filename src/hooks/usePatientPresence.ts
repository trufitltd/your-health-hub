...
export const usePatientPresence = () => {
  const [presenceMap, setPresenceMap] = useState<Record<string, PresenceStatus>>({});
  const [channel, setChannel] = useState<RealtimeChannel | null>(null);

  useEffect(() => {
    const presenceChannel = supabase.channel('patients-presence');
...
export const trackPatientPresence = async (userId: string, status: PresenceStatus = 'online') => {
  const channel = supabase.channel('patients-presence');
