-- Add doctor preferred UI language for default app experience.
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conname = 'doctor_registrations_preferred_language_check'
  ) THEN
    ALTER TABLE public.doctor_registrations
      ADD CONSTRAINT doctor_registrations_preferred_language_check
