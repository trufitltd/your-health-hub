-- Fix respond_appointment_reschedule to set updated_at on approve
-- so realtime subscriptions fire and frontends pick up the new date/time
  ...
  IF v_upgrade_amount > 0 THEN
    ...
    ELSE
      ...
      INSERT INTO public.patient_wallet_transactions (
        patient_id, appointment_id, amount, direction,
        transaction_type, status, narration
      ) VALUES (
        v_patient_id, p_appointment_id, v_upgrade_amount, 'debit',
        'booking_wallet_use', 'completed', 'Reschedule upgrade payment from wallet'
      );
    END IF;
  END IF;
